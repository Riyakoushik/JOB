...
import { USER_PROFILE, MOCK_HRS } from '@/data/constants';

export default function HRContactsTab({ showToast }) {
    const [savedContacts, setSavedContacts] = useLocalStorage('jh_hr_contacts', []);
    ...
    // Merge mock contacts with user-saved contacts (saved first)
    const allContacts = [...savedContacts, ...MOCK_HRS.filter(m => !savedContacts.some(s => s.id === m.id))];

    const filtered = allContacts.filter(h => {
    ...
    const saveHR = () => {
        ...
        const newContact = { ...form, id: `h-${Date.now()}`, isVerified: true, avatarColor: '#3b82f6' };
        setSavedContacts([newContact, ...savedContacts]);
        setShowAdd(false);
        setForm({ name: '', company: '', designation: '', email: '', phone: '', linkedinUrl: '', roleHiring: '' });
            ...
            {showAdd && (
                <Card className="animate-fade-in-up bg-zinc-900/50 backdrop-blur-md">
                    <CardHeader>
                        ...
                        <CardDescription>Add a recruiter you've connected with</CardDescription>
                    </CardHeader>
